...
const connection = new Connection('YOUR_API_KEY', 'confirmed');
...
async function fetchAndProcessTransactions(wallet, walletSet, depth, delay) {
    ...
    console.log(`Fetching transactions for wallet: ${wallet} (up to ${depth} transactions)`);
    ...
    for (const tx of transactions) {
        ...
        for (const instr of instructions) {
            ...
            if (parsed?.type === 'transfer') {
                ...
                if (walletSet.has(source) && walletSet.has(destination) && source !== destination) {
                    // Determine if it's native SOL or SPL token
                    const splOrNative = programId.toString() === '11111111111111111111111111111111' ? 1 : 0;
...
async function main(filePath, depth, delay) {
    const wallets = fs.readFileSync(filePath, 'utf-8')
        .split('\n')
        .map(line => line.trim())
...
if (args.length < 2) {
    console.error('Usage: node gator-cli.js <wallets_file> <depth> [delay_in_ms]');
    process.exit(1);
}
